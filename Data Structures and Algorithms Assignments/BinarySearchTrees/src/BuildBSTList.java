...
public class BuildBSTList {
			...
			BinarySearchTreeList<Integer> bstList)
	{
		...
		buildBalancedRec(tempArr, 0, tempArr.length, bstList);
	}
	...
	{		
		if(start != end ) {
			...
			bstList.add(tempArr[index]);
			
			
			buildBalancedRec(tempArr, start, index, bstList);
			buildBalancedRec(tempArr, index+1, end, bstList);
		}
	}
