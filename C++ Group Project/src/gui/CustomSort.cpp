...
void CustomSort::setNext(int fieldNum, int currentIndex) {
    ...
    // This will clear any dropdowns below one that is selected if the no value is selected
    if(currentIndex == 0){
        fieldBoxes.at(fieldNum+1)->clear();
    }
}

void CustomSort::on_field_0_currentIndexChanged(int index) {
    setNext(0, index);
    if (fieldBoxes.at(0)->currentText().contains("Date")){
        QMessageBox::critical(this, "", "Sorry, cannot sort by date first.");
        fieldBoxes.at(1)->clear();
        fieldBoxes.at(0)->setCurrentIndex(0);
